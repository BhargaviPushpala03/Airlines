...
public class InternationalFlight extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            Connection con = DriverManager.getConnection("jdbc:mysql://localhost:3306/project", "root", "Nanna@123");
            ...
            response.getWriter().println("body {font-family: Arial, sans-serif; background-color: #e3f2fd; margin: 0; padding: 0; padding-top: 60px;}"); // Light blue background
            response.getWriter().println("h2 {text-align: center; color: #1e88e5; margin-top: 20px;}"); // Darker blue for headings
            ...
            response.getWriter().println("th {background-color: #1976d2; color: white;}"); // Header color changed to dark blue
            response.getWriter().println("tr:hover {background-color: #bbdefb;}"); // Light blue hover
            response.getWriter().println(".footer {text-align: center; padding: 20px;}");
            response.getWriter().println(".footer a {color: #fff; font-weight: bold; text-decoration: none; background-color: #1976d2; padding: 10px 20px; border-radius: 5px; border: 1px solid #1976d2; display: inline-block;}"); // Footer link color changed to dark blue
            response.getWriter().println(".footer a:hover {background-color: #1565c0;}"); // Footer link hover
            response.getWriter().println(".book-button {background-color: #1976d2; color: white; border: none; padding: 10px 20px; text-align: center; text-decoration: none; display: inline-block; font-size: 14px; margin: 4px 2px; cursor: pointer; border-radius: 4px;}"); // Button color changed to dark blue
            response.getWriter().println(".book-button:hover {background-color: #1565c0;}"); // Button hover
