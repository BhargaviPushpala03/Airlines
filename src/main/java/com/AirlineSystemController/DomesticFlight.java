...
public class DomesticFlight extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            response.getWriter().println("body {font-family: Arial, sans-serif; background-color: #e3f2fd; margin: 0; padding: 0; padding-top: 60px;}"); // padding for the fixed menu
            response.getWriter().println("h2 {text-align: center; color: #1e88e5; margin-top: 20px;}");
            ...
            response.getWriter().println("th {background-color: #1976d2; color: white;}");
            response.getWriter().println("tr:hover {background-color: #bbdefb;}");
            response.getWriter().println(".footer {text-align: center; padding: 20px;}");
            response.getWriter().println(".footer a {color: #fff; font-weight: bold; text-decoration: none; background-color: #1976d2; padding: 10px 20px; border-radius: 5px; border: 1px solid #003366; display: inline-block;}");
            response.getWriter().println(".footer a:hover {background-color: #1565c0;}");
            response.getWriter().println(".book-button {background-color: #1976d2; color: white; border: none; padding: 10px 20px; text-align: center; text-decoration: none; display: inline-block; font-size: 14px; margin: 4px 2px; cursor: pointer; border-radius: 4px;}");
            response.getWriter().println(".book-button:hover {background-color: #1565c0;}");
